  ...
  async analyze(files: SourceFile[], cache?: AnalysisCache): Promise<AnalysisResult> {
    ...
    for (const parser of this.parsers) {
      ...
      if (matchedFiles.length > 0) {
        try {
          if (cache) {
            const cachedResult = cache.getParserResult(parser.name, matchedFiles);
            if (cachedResult) {
              this.logger.debug(`Using parser cache for "${parser.name}" (${matchedFiles.length} files)`);
          ...
          if (cache) {
            cache.setParserResult(parser.name, matchedFiles, result);
          }
        } catch (error) {
